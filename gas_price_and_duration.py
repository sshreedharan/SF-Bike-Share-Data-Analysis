...
def visualization_gas_price():
    ...
    plot1 = hv.Scatter(gas_rental, 'gas_price', 'rental_num')\
        .options(width=800,height=500,color='r',size=2, title="Relatonship between Number of Rentals and Gas Price (Dollars per Gallon)")
...
def visualization_duration():
    ...
    plt.ylabel("duration")
    plt.plot(trip[trip.duration < 10000].loc[:, 'start_time'], trip[trip.duration < 10000].loc[:, 'duration'], 'o', color='blue', markersize=1)
